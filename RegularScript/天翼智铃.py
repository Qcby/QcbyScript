"""
天翼智铃 v1.1.0（青龙多账号聚合推送版）

功能：自动执行中国电信首页「天翼智铃 / 2026新年星辰」活动，包含账密登录、制作视频、AI 图片生成、抽奖、红包余额查询与满额兑换，支持多账号执行，执行结束后统一聚合推送。

配置说明：
1. 账号变量：
   chinaTelecomAccount                              电信账密变量
   - 单账号格式：手机号#密码 或 手机号@密码
   - 多账号支持使用 &、英文逗号、中文逗号或换行分隔
   - 示例：13800138000#password&13900139000@password
...
ACCOUNTS_STR = (
    os.environ.get("chinaTelecomAccount", "")
)
...
def split_multi_values(raw: str) -> list:
    """统一多账号分隔符：&、英文逗号、中文逗号、换行。"""
    return [item.strip() for item in re.split(r"[&,\n，]+", raw.replace("\r", "\n")) if item.strip()]


def parse_accounts(accounts_str: str):
    accounts = []
    invalid_entries = []
    for part in split_multi_values(accounts_str):
        separator = "#" if "#" in part else "@" if "@" in part else ""
        if separator:
            phone, pwd = part.split(separator, 1)
        else:
        ...
        if phone and pwd:
            ...
            invalid_entries.append(part)
    return accounts, invalid_entries
...
def mask_account(account: str) -> str:
    ...
    if not account:
        return "未配置"
    if len(account) >= 7 and account[:3].isdigit():
        return f"{account[:3]}****{account[-4:]}"
    if len(account) <= 4:
        return account[0] + "***" if account else "未配置"
    return f"{account[:2]}***{account[-2:]}"


def append_notify_result(item: dict) -> None:
    GLOBAL_NOTIFY_BUFFERS.append(item)
...
def build_notify_report() -> str:
    ...
    total_red_packet = sum(float(item.get("red_packet") or 0) for item in GLOBAL_NOTIFY_BUFFERS)
    ...
    for item in GLOBAL_NOTIFY_BUFFERS:
        ...
        lines.extend([
            f"{account_icon} 【账号{item.get('index', '-')}】{item.get('account', '未知账号')}",
            f"{status_icon} 状态：{status_text}",
        ])

        if ok:
            lines.extend([
                f"🎬 视频任务：{item.get('task1_status', '未知')}",
                f"🤖 AI任务：{item.get('task2_count', 0)}/3",
                f"🎰 抽奖：{item.get('lottery_success', 0)}/{item.get('lottery_total', 0)}",
                f"🧧 本次红包：{float(item.get('red_packet') or 0):.2f} 元",
                f"💰 可兑余额：{item.get('balance', '未知')} 元",
                f"🏦 兑换状态：{item.get('redeem_status', '未触发')}",
            ])
            awards = item.get("awards") or []
            if awards:
                lines.append(f"🎁 奖品：{', '.join(awards[:6])}")
        else:
...
def userLoginNormal(phone, password):
    ...
    r = ss.post('https://appgologin.189.cn:9031/login/client/userLoginNormal', json=payload, timeout=30)
    data = r.json()
...
def process_new_year_lottery(phone: str, password: str, account_idx: int = 0, total_accounts: int = 0):
    ...
    if not user_id or not login_token:
        print("❌ 登录响应缺少 userId/token")
        return fail("登录响应缺少 userId/token")
    ticket = get_ticket(phone, user_id, login_token)
    if not ticket:
        print("❌ 获取ticket失败")
        return fail("获取 ticket 失败")
    print("✓ 获取ticket成功")
    print("\n[3] SSO登录...")
    token, cookies = sso_login_v2(ticket)
    if not token:
        print("❌ SSO登录失败")
        return fail("SSO 登录失败")
    print(f"✓ SSO登录成功")
        ...
        try:
            ...
            if balance_float >= 2.0:
                ...
                if redeem_result.get("code") == "0000":
                    ...
                    balance_cost = new_balance
                    redeem_status = f"兑换成功 {extract_value} 元"
                else:
                    msg = redeem_result.get("desc", redeem_result.get("message", "未知错误"))
                    print(f"  ✗ 兑换失败: {msg}")
                    redeem_status = f"兑换失败：{msg}"
            else:
                print(f"  ℹ️  需满2元才能兑换（还差{2.0 - balance_float:.2f}元）")
                redeem_status = f"未满2元，差{2.0 - balance_float:.2f}元"
        except Exception as e:
            print(f"  ⚠ 处理失败: {e}")
            redeem_status = f"余额处理失败：{e}"
    else:
        msg = balance_result.get("desc", balance_result.get("message", "未知错误"))
        print(f"  ✗ 查询失败: {msg}")
        balance_cost = "查询失败"
        redeem_status = f"余额查询失败：{msg}"
    ...
    ok = task1_success or task2_success_count > 0
    result_summary.update({
        "ok": ok,
        "status_text": "执行成功" if ok else "任务未完成",
        "message": "" if ok else "视频任务和 AI 任务均未成功",
        "task1_status": "成功" if task1_success else "失败",
        "task2_count": task2_success_count,
        "lottery_success": lottery_success,
        "lottery_total": final_times,
        "red_packet": total_red_packet,
...
def main():
    accounts, invalid_entries = parse_accounts(ACCOUNTS_STR)

    for idx, invalid_entry in enumerate(invalid_entries, 1):
        append_notify_result({
            "index": f"配置{idx}",
            "account": mask_account(invalid_entry),
            "ok": False,
            "status_text": "配置错误",
            "message": "账号格式错误，应为 手机号#密码 或 手机号@密码",
        })
    ...
    if not accounts:
        ...
        if not invalid_entries:
            append_notify_result({
                ...
                "ok": False,
                "status_text": "配置错误",
                "message": "未配置 chinaTelecomAccount",
            })
        dispatch_notify()
        return
    ...
    for idx, (phone, password) in enumerate(accounts):
        try:
            result = process_new_year_lottery(phone, password, idx + 1, total)
        except Exception as exc:
            result = {
                "index": idx + 1,
                "account": mask_account(phone),
