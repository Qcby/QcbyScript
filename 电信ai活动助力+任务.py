"""
电信 AI 活动助力任务 v1.1.0（互助聚合推送版）

功能：中国电信 AI 活动账号登录、预助力、创作任务、积分兑奖、抽奖与话费中奖记录领取；支持多账号互相助力，执行结束后统一聚合推送。

配置说明：
1. 账号变量：
   AI096_ACCOUNTS                                  必填，中国电信账号变量
   - 格式：手机号#密码
...
TEST_ACCOUNTS = ""
ACCOUNTS_STR = os.environ.get("AI096_ACCOUNTS") or os.environ.get("CAILING_ACCOUNTS", TEST_ACCOUNTS)
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
ACCOUNTS_FILE = os.path.join(SCRIPT_DIR, "accounts.txt")
MAX_EXPIRY_DATE = datetime.date(2026, 6, 30)
CHANNEL_ID = "156000008353"
ACTIVITY_ID = "ai096"
TEMPLATE_ID = "ve_3949"
...
def append_notify_result(index, account, ok, status, message=""):
    GLOBAL_NOTIFY_BUFFERS.append({
        "index": index,
        "account": mask_phone(str(account)) if str(account or "").isdigit() else str(account or ""),
        "ok": bool(ok),
...
def build_notify_report():
    ...
    for item in GLOBAL_NOTIFY_BUFFERS:
        ...
        if ok:
            lines.append(f"🤝 互助/任务：{item.get('message') or '完成'}")
        else:
...
def build_mutual_pre_assist_targets(current_phone, invitation_codes):
    targets = []
    seen_codes = set()
    for target_phone, invitation_code in (invitation_codes or {}).items():
        if target_phone == current_phone or not invitation_code or invitation_code in seen_codes:
            continue
        seen_codes.add(invitation_code)
        targets.append({
            "code": invitation_code,
            "label": f"互助目标:{mask_or_plain(target_phone)}",
...
def run():
    ...
    pre_assist_targets = prompt_pre_assist_targets()
...
def prompt_pre_assist_targets():
    # 外部预助力已按要求彻底关闭，只保留本次账号之间互相助力。
    return []


def run():
    if not ensure_runtime_ready():
        append_notify_result(1, SCRIPT_TITLE, False, "config_error", "运行依赖不完整")
        dispatch_notify()
        return
    ...
    accounts = [(phone, password) for phone, password, _, _, is_valid in loaded_accounts if is_valid]
    if not accounts:
        append_notify_result(1, "AI096_ACCOUNTS", False, "config_error", "没有可用账号，请检查 AI096_ACCOUNTS 或 accounts.txt")
        dispatch_notify()
        raise ValueError("没有可用账号，请检查 AI096_ACCOUNTS 或 accounts.txt")
    ...
    extra_pre_assist_targets = prompt_pre_assist_targets()
    draw_limit = prompt_lottery_limit()
    print(f"\n本次抽奖设置: {'全抽' if draw_limit is None else draw_limit}")

    print("\n开始获取本次账号互助邀请码...")
    invitation_codes = collect_invitation_codes(accounts, proxy_assignment)
    if invitation_codes:
        print("本次互助关系:")
        for phone, _ in accounts:
            targets = [mask_or_plain(other_phone) for other_phone in invitation_codes if other_phone != phone and invitation_codes.get(other_phone)]
            print(f"  {mask_or_plain(phone)} -> {', '.join(targets) if targets else '无可用互助目标'}")
    else:
        print("⚠️ 未获取到任何互助邀请码，本次将只执行任务，不进行互助")
    ...
    for idx, (phone, password) in enumerate(accounts, start=1):
        ...
        if proxy_manager.proxy_source == proxy_manager.SOURCE_API:
            ...
            if not current_proxy:
                masked_phone = mask_phone(phone)
                message = "未获取到可用代理，跳过账号"
                print(f"  {message}: {masked_phone}")
                append_notify_result(idx, phone, False, "failed", message)
                continue
            used_proxies.add(current_proxy)
            print(f"  当前账号使用代理: {current_proxy}")

        mutual_pre_targets = build_mutual_pre_assist_targets(phone, invitation_codes)
        pre_assist_targets = mutual_pre_targets + extra_pre_assist_targets
        assist_targets = [target_phone for target_phone in invitation_codes if target_phone != phone]
        success = run_single_account(
            phone,
            password,
            current_proxy,
            assist_mode="specified" if assist_targets else "off",
            assist_targets=assist_targets,
        ...
        if success:
            append_notify_result(idx, phone, True, "success", "账号互助/任务处理完成")
            continue

        if proxy_manager.proxy_source != proxy_manager.SOURCE_API:
            append_notify_result(idx, phone, False, "failed", "账号处理失败")
            continue
        ...
        for retry_round in range(2, ACCOUNT_PROXY_RETRY_LIMIT + 1):
            ...
            retry_success = run_single_account(
                phone,
                password,
                current_proxy,
                assist_mode="specified" if assist_targets else "off",
            ...
            if retry_success:
                break
        append_notify_result(idx, phone, bool(retry_success), "success" if retry_success else "failed", "账号互助/任务处理完成" if retry_success else "账号处理失败")
