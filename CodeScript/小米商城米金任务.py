...
# 默认成功也推送；如需仅失败推送，显式设置 NOTIFY_ON_FAIL_ONLY=true。
NOTIFY_ON_FAIL_ONLY = os.getenv("NOTIFY_ON_FAIL_ONLY", "false").strip().lower() in ("1", "true", "yes")
...
def dispatch_notify() -> None:
    if GLOBAL_NOTIFY_BUFFERS:
        # 兼容旧版青龙配置：配置 BARK_PUSH 时直接走本脚本 Bark 推送。
        if BARK_PUSH.strip():
