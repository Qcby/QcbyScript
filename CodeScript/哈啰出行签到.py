...
"""
哈啰出行签到 v1.1.0（mywc网关聚合推送版）

功能：自动执行哈啰出行小程序登录、每日签到、普通宝箱领取和金币余额查询，支持多账号执行，执行结束后统一聚合推送。
...
1. 微信 code 网关：
   ...
   - 脚本会自动拼接 /mywc
   - 请求格式：GET {网关}/mywc?wxid=账号标识&appId=wxb937e3d0b3ca117e
   - 请求头：auth=账号标识

2. 账号变量：
   hello_wxid 或 HELLO_WXID                         推荐，哈啰出行专属账号变量
   - 多账号支持使用 &、英文逗号、中文逗号或换行分隔
   - 示例：wxid_a&wxid_b 或 wxid_a,wxid_b
   - 兼容旧变量 HELLOBIKE_WXID / HELLO 读取
...
HELLO_WXID_RAW = (
    os.getenv("hello_wxid")
    or os.getenv("HELLO_WXID")
    or os.getenv("HELLOBIKE_WXID")
    or os.getenv("HELLO")
    or ""
)
...
def run_account(index: int, total: int, wxid: str) -> Dict[str, Any]:
    ...
    try:
        sign_resp = api_post(wxid, SIGN_URL, token, proxies, SIGN_PAYLOAD)
        ...
        box_resp = api_post(wxid, BOX_URL, token, proxies, BOX_PAYLOAD)
        ...
        point_resp = api_post(wxid, POINT_URL, token, proxies, POINT_PAYLOAD)
...
def main() -> None:
    wxids = parse_accounts(HELLO_WXID_RAW)
    log_title(len(wxids))

    if not wxids:
        append_notify_result(empty_result("未配置", "请配置 hello_wxid 或 HELLO_WXID"))
