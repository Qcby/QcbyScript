...
1. 微信 code 网关：
   ...
   - 脚本会自动拼接 /mywc
   - 请求格式：GET {网关}/mywc?wxid=账号标识&appId=wxada7aab80ba27074
   - 请求头：auth=账号标识

2. 账号变量：
   qqmusic_wxid 或 QQMUSIC_WXID                     推荐，QQ音乐专属账号变量
   - 多账号支持使用 &、英文逗号、中文逗号或换行分隔
   - 示例：wxid_a&wxid_b&wxid_c 或 wxid_a,wxid_b,wxid_c
   - 兼容旧变量 QQMUSIC / QQ_MUSIC_WXID 读取
...
QQMUSIC_WXID_RAW = (
    os.getenv("qqmusic_wxid")
    or os.getenv("QQMUSIC_WXID")
    or os.getenv("QQ_MUSIC_WXID")
    or os.getenv("QQMUSIC")
    or ""
)
...
def get_code(wxid: str) -> str | None:
    ...
    for attempt in range(1, 3):
        try:
            ...
            code = str(
                data.get("code")
                or safe_data(data).get("code")
                or safe_data(safe_data(data)).get("code")
                or ""
            ).strip()
            err_no = data.get("err")
            if err_no not in (None, 0, "0") or not code or code.lower() == "null":
...
def main() -> None:
    wxids = parse_accounts(QQMUSIC_WXID_RAW)
    log_title(len(wxids))
    ...
    if not wxids:
        append_notify_result({
            ...
            "redPacketMsg": "-",
            "balance": "-",
            "error": "请配置 qqmusic_wxid 或 QQMUSIC_WXID",
        })
    ...
    if not WX_SERVER_URL:
        for wxid in wxids:
            append_notify_result({
                ...
                "redPacketMsg": "-",
                "balance": "-",
                "error": "请配置 wx_server_url 或 WX_SERVER_URL",
            })
        dispatch_notify(SCRIPT_TITLE, build_notify(GLOBAL_NOTIFY_BUFFERS))
        return
